...
test("#nextChar 1", () => {
  ...
  expect(currentParser.currChar()).toEqual("1");
});

test("#nextChar 2", () => {
  const currentParser = new HtmlParser(5, "12abcあいう");
  expect(currentParser.currChar()).toEqual("あ");
});
...
test("parse attributes 1", () => {
  ...
  expect(currentParser.parseAttributes()).toEqual(
    new Map([
      ["foo", "foo1"],
      ["bar", "bar2"],
    ])
...
test("parse nodes 2", () => {
  ...
  expect(currentParser.parseNodes()).toEqual([
    Element(
      ...
      [Text("bananas "), Element("div", new Map([["moo", "moo3"]]), [Text("apples")])]
    ),
  ]);
...
test("htmlParse 1", () => {
  ...
  expect(htmlParse(html)).toEqual(Element("html", new Map([]), [Element("body", new Map([]), [])]));
});

test("htmlParse 2", () => {
  const html = `<div>bananas apples</div> <div>oranges</div>`;
  expect(htmlParse(html)).toEqual(
    Element("html", new Map([]), [
      Element("div", new Map(), [Text("bananas apples")]),
      Element("div", new Map(), [Text("oranges")]),
    ])
  );
});
